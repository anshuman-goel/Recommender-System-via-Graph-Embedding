...
def process(args):
    ...
    # YOUR CODE HERE
    model = Word2Vec(graph.build_deepwalk_corpus(G,args.number_walks,args.walk_length,alpha=0,rand=random.Random(0)),size=args.representation_size,window=args.window_size,min_count=0,workers=args.workers)                     
...
def predict_rating(model, nodedict, user, movie):
    ...
    rating=3
    for i in range(1,6):
        sim=model.similarity(nodedict[user].id,nodedict[movie+'_'+str(i)].id)
        if sim>max_sim:
            max_sim=sim
            rating=i
