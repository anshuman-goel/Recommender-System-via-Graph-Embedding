...
def records_to_graph():
    ...
    for key,value in movies.iteritems():
        node=Node(id,value.name,'movie')
        nodelist.append(node)
        nodedict[key]=node
        id+=1
        for i in range(1,6):
            node=Node(id,key+'_'+str(i),'rating')
            nodelist.append(node)
            nodedict[key+'_'+str(i)]=node
            id+=1

    for director in directors:
        node=Node(id,director,'director')
        nodelist.append(node)
        nodedict[director]=node
        id+=1

    for actor in actors:
        node=Node(id,actor,'actor')
        nodelist.append(node)
        nodedict[actor]=node
        id+=1

    for genre in genres:
        node=Node(id,genre,'genre')
        nodelist.append(node)
        nodedict[genre]=node
        id+=1

    for user in ratings:
        node=Node(id,user,'user')
        nodelist.append(node)
        nodedict[user]=node
        id+=1
    ...
    for user,user_obj in ratings.iteritems():
        for movie,rating in user_obj.iteritems():
            nodedict[user].neighbors.append(nodedict[movie+'_'+str(rating)])
            nodedict[movie+'_'+str(rating)].neighbors.append(nodedict[user])

    for movie,movie_obj in movies.iteritems():
        if movie_obj.director!=None:
            nodedict[movie].neighbors.append(nodedict[movie_obj.director])
            nodedict[movie_obj.director].neighbors.append(nodedict[movie])
    ...
    for movie in movies:
        for i in range(1,6):
            nodedict[movie].neighbors.append(nodedict[movie+'_'+str(i)])
            nodedict[movie+'_'+str(i)].neighbors.append(nodedict[movie])
